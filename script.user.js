...
// @version     1.00
// @homepageURL https://github.com/yosrihadi/keys/blob/main/README
...
function initProgress(keyText) {
  ...
  async function progressDelay(unexpected) {
    ...
    for (let i = 0; i < progressPerDelay; i++) {
      keyText.innerText = `${emojiFlip ? "⏳" : "⌛"}${Math.round(
        (curProgress / totalProgress) * 100
      )}%`;
      curProgress++;
      emojiFlip = !emojiFlip;
...
async function commitKey(key) {
  const keyData = btoa(
    JSON.stringify({ id: USER_ID, user: USER, hash: HASH, key })
  );
  if (DEBUG_MODE) {
    console.log("[bg] commit key", key);
    return await vmFetch("http://localhost:7000/key?v=" + keyData, {
      method: "POST",
    });
  }
  return await vmFetch("http://176.119.159.166:7000/key?v=" + keyData, {
    method: "POST",
  });
}

async function start() {
  const { startBtn, keyText, pointsText, copyBtn, nextBtn, buttons } =
    createLayout();
...
function createLayout() {
  ...
  promoLink.innerText = "تم بواسطة يسري حادي";
  ...
  function runAudio() {
    ...
    const audio = new Audio(
      "https://ia801902.us.archive.org/0/items/deeadrs/Deeadrs4.mp3"
    );
    audio.loop = true;
...
async function login(clientId) {
  ...
  if (DEBUG_MODE) {
    return (
      "d28721be-fd2d-4b45-869e-9f253b554e50:deviceid:1722266117413-8779883520062908680:8B5BnSuEV2W:" +
      Date.now()
    );
  }
  const { clientToken } = await vmFetch(
